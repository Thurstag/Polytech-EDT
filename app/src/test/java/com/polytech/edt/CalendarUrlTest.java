...
public class CalendarUrlTest {
    ...
    public void UrlMethodTest() throws Exception {
        URL url = new CalendarUrl(Collections.singletonList(Resource.APP3_TC_GROUP_C), scope).url();
        ...
        Assert.assertEquals(1, parameters.stream().filter(new Predicate<NameValuePair>() {
            ...
            public boolean test(NameValuePair nameValuePair) {
                return nameValuePair.getName().equals("resources") && nameValuePair.getValue().equals(Resource.APP3_TC_GROUP_C.id() + "");
            }
        }).count());
        Assert.assertEquals(1, parameters.stream().filter(new Predicate<NameValuePair>() {
            @Override
